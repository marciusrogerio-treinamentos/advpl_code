...
If nTypeTrans == TRANS_RECEIVE
   If cTypeMessage <> EAI_MESSAGE_WHOIS
      ...
      If !Empty(cNum) 
         If !File("06RSPNF\"+cNum+".txt")
            If !lDelete
               cIdInt := Alltrim(StrZero(Val(GETSX8NUM("SWN","WN_DOC")),AVSX3("WN_DOC",AV_TAMANHO))) //Tamanho do WT_NUMERP na Origem
               ConfirmSX8()
               //aTit := {"1",cIdInt}
               //MemoWrite("02RSPNF\"+cNum+".txt", StrMerge(aTit))
         ...
         Else 
            ...
            //aTit := EasyStrSplit(MemoRead("02RSPNF\"+cNum+".txt"),";")
