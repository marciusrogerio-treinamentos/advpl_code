...
Static Function GeraVerbas(cGrupo1, cGrupo2, cGrupo3)
	...
	BeginSQL ALIAS cAliasQry
		...
		AND SRD.%NotDel%
		AND SRA.%NotDel%
		ORDER BY RD_FILIAL, RD_PROCES, RD_MAT, RD_CC
	EndSQL
...
Static Function GravaPgt(aPagamento)
	...
	For nI := 1 To Len(aPagamento)
		...
		If ! RGB->(dbSeek(aPagamento[nI][1] + aPagamento[nI][2] + aPagamento[nI][3] + aPerAtual[1][1] + aPerAtual[1][2]))
			RecLock("RGB", .T.)
				RGB->RGB_FILIAL	:= aPagamento[nI][1]
				RGB->RGB_MAT	:= aPagamento[nI][2]
				...
				RGB->RGB_TIPO1	:= PosSRV(aPagamento[nI][3], aPagamento[nI][1], "RV_TIPO" )
		...
		Else
			...
			aAdd(aLogRGB, "Funcionário" + ": " + aPagamento[nI][1] + " - " + aPagamento[nI][2])
		EndIf
