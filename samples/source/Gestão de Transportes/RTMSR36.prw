...
DUY->(dbSetOrder(1))
If DUY->( MsSeek( xFilial("DUY") + cAux)) .And. Empty(DUY->DUY_CODMUN)
    lRet    := .F. 

    If Upper(cPar) == "MV_PAR04" .Or. Upper(cPar) == "MV_PAR05" 
        Help("",1, ".RTMSR3604.",,,3,0) //-- Informe o código da região de origem De/Até. Apenas serão aceitos Regiões que contenham municípios vinculados.
    Else
        Help("",1, ".RTMSR3606.",,,3,0) //-- Informe o código da região de destino De/Até. Apenas serão aceitos Regiões que contenham municípios vinculados.
    EndIf
